...
<head>
  ...
  <title><?php echo APP_TITLE; ?> | All Employies</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Employes
        <small>All Employes are Listed Here.</small>
      </h1>
      <ol class="breadcrumb">
      ...
      <div class="box">
        <div class="box-body">
          ...
          <table class="table table-striped">
                ...
                <?php
                if(isset($model['employes']) && count($model['employes']) > 0)
                {
                  foreach($model['employes'] as $row )
                  {
                ?>
                <tr>
                  <td><?php echo $row['id']; ?></td>
                  <td><img style="height:100px;"  class="profile-user-img img-responsive img-circle" src="/Uploads/<?php echo $row['image']; ?>" alt="User profile picture"></td>
                  <td><?php echo $row['name']; ?> ( <?php echo $row['email']; ?> )</td>
                  <td>
                    <?php echo $departments_model->get_by_id($row['department'])['name'];?>
                  </td>
                  <td>
                    <?php 
                    $hours = $servicelog_model->get_total_hours_by_employe($row['id'])['total'];
                    if($hours == ''){
                  ...
                  <td><a href="<?php echo url_to(array("Employes","Edit",$row['id'])); ?>" class="btn btn-xs btn-primary"><i class="fa fa-pencil"></i></a><a href="<?php echo url_to(array("Employes","Delete",$row['id'])); ?>" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a></td>
                </tr>
                
                <?php } } else { ?>
                <tr><td colspan=4><center><h4>No Employes in the system...!</h4></center></td></tr>
                <?php } ?>
              </tbody></table>
        </div>
      </div>
